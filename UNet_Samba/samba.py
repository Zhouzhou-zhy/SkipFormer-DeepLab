...
class Samba(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        纯净版 Backbone forward
        输入: x (B, C, H, W) - 原始图像
        输出: tuple(outs) - 包含 4 个阶段特征图的元组
        """
        # 1. 提取 Patch Embeddings
        x, H, W = self.forward_embeddings(x)

        # 2. 通过 4 个 Stage 提取特征
        # 不需要原来的 mix_token 和 bbx1 等数据增强逻辑
...
if __name__ == "__main__":
    ...
    backbone = Samba(in_chans=3, embed_dims=[64, 128, 256, 512], depths=[3, 4, 6, 3])
    backbone.to(device)
    ...
    for i, feat in enumerate(features):
        print(f"Stage {i+1} 特征维度: {feat.shape}")
